...
- we get rid of recognition using yoloface, as we could see the cropping is very bad (but can do with own model)  
- we write it clearer with non for self explanations
- instead of stating a threshold and putting a DK when it is reached, we will be simply labelling it directly
- we do not implement yet the small horizontalize that'll be for later version
...
class Temporary():
    #Explanations here
    # change_array : it is the bool array for the instructions to recheck the pets' identity
    # present: currently present pet
    # reverse: reverse map of presnet
    # get_rid: array of the detection to get rid of
    # still: tmp array 
    def __init__(self):
        self.change_array = []
        self.present = dict()
...
def video_yolos(filename,fps = 25.0,verbose = False):
    ...
    while True:
        ...
        for i in range(pets.size):
            ...
            #i is index in pets array and idx is index in r 
            tmp.present[i] = idx
            tmp.reverse[idx] = i
        ...
        for i in range(pets.previous_size):
            ...
            t = tmpPet.t
            if (i not in tmp.present.keys() or i in tmp.still) and cur - t > lapse:

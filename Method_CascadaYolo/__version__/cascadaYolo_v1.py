...
def vid_yolo(filename,fps = 25.0):
    ...
    net=  load_net("cfg/cat-dog-yolov3-tiny.cfg".encode("utf-8"), "weights/cat-dog-yolov3-tiny_90000.weights".encode("utf-8"), 0)
    ...
    while (True):
        ...
        if not ret:
            break
        r = detect_s(net,meta, frame,thresh,nms = 0.1)
...
def video_yolos(filename,fps = 25.0,verbose = False):
    ...
    #Prep
    net = load_net("cfg/cat-dog-yolov3-tiny.cfg".encode("utf-8"), "weights/front_prof_90k.weights".encode("utf-8"), 0)
    meta = load_meta("cfg/cat-dog-obj.data".encode('utf-8'))
    net_big =  load_net("cfg/yolov3-tiny.cfg".encode("utf-8"), "yolov3-tiny.weights".encode("utf-8"), 0)
    meta_big = load_meta("cfg/coco.data".encode('utf-8'))
    ...
    while True:
        ...
        #  we do this to deal with the case where a pet leaves the camera
        if len(seen) > len(r):
            
            # in order to keep continuity
            for i in range(len(seen)):
                if time.time() - seen[i][-1] > lapse and seen[i] not in get_rid: get_rid.append(seen[i])
        
        for thing in get_rid: seen.remove(thing)  
        ...
        for i in range(len(seen)):
            idx, dist = closest_pair(seen[i],r)
        ...
        #having the matches, we deal with them, either appending them to seen array if not already seen, if seen then update its coordinates
        cur = time.time()
        for i in range(len(r)):
            if i not in present1: #there is a new pet recognized that appeared
                ...
                tmp[(x,y,w,h)] = (None,0)
                change = True #as we have detected a new pet, then need to try recognizing him
            else:
        ...
        """
        # when two points too close then compute again recognition #arbitrary 5 pixels distance
        if change == False:
            for (R,P,(x,y,w,h)) in r:
                for (R1,P1,(x1,y1,w1,h1)) in r:
                    if P != P1 and sqrt((x-x1)**2+(y-y1)**2) < 5:
                        change = True
                        if (x,y,w,h) in tmp: del tmp[(x,y,w,h)] # we delete the recognition
        """
        ...
        for (R,P,(x,y,w,h)) in r: #We iterate over what body we see
            ...
            x1,y1,w1,h1 = x,y,w,h
            if (x,y,w,h) in tmp:
                if tmp[(x,y,w,h)][0] == None: change = True 
            ...
            if change == False:
                ...
                if random.random() < 0.05: change = True
            if change:
                ...
                if len(cats_ext) == 0: 
                    ...
                    #print(cats_ext)
                    name, conf = model.predict(gray[y:y+h,x:x+w])
                    cv2.imwrite('testing/'+str(number)+'file.jpg',gray[y:y+h,x:x+w])
                    conf = int(100*(1-(conf)/400))
                    tmp[(x1,y1,w1,h1)] = name,conf
                    print('conf2 '+str(conf))
                    change = True
                    number += 1
        for obj in seen:
            x,y,w,h,t = obj
            ...
            if conf >= 83 : img = cv2.putText(img,full[name],(Xc-10,Yc-10),cv2.FONT_HERSHEY_COMPLEX,1,(0,0,255),2)

...
"""this is supposed to be a script that helps augmenting the data amount, it is not used by the customer (so did not do catching error/exception)
...
def shift_image_LR(file,cur,verbose = False): #we shift, images is the link to it, filename
    ...
    test_list = open('order/cat-dog-test.txt','a')    
    train_list = open('order/cat-dog-train.txt','a')
...
"""To have the center of the pic as (0,0) """ 

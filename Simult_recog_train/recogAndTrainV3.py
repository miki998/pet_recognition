...
- Add the horizontalization for more facial detection by haar
- Get rid of prediction using Yoloface
- Some normalization of the error according to original size compared to original size of training set, can also normalize using the predictions of others#TODO
- Does not require a starting model anymore, we will just be writing it from scratch if needed
- + Will implement in the future, a higher requirement of the cropped images for real time training, not just some random similarish pics
...
class HaarCascade:
    ...
    def detect(self,img,dim):
        return self.__cat_cascade.detectMultiScale(img,scaleFactor=self.__SF,minNeighbors=self.__N,minSize=dim)
...
class LBPHModel:
    def __init__(self,filename):
        ...
        try :
            ...
            if filename[-4:] != '.xml': 
                print('Wrong extension boy')
                sys.exit(0)
            self.exist = 0
            print("Alright, I guess you are starting anew/ from scratch you sneaky boy")
...
def video_yolos(filename,fps = 25.0,verbose = False): 
    ...
    # Model Prep
    yoloface = Yoloface(thresh=0.4)
    yolobody = Yolobody(thresh=0.1)
    yoloeye = Yoloeye(thresh=0.5)
    cascadaExt = HaarCascadeExt(SF=1.03,N=5)
    model = LBPHModel('new.xml')
    ...
    while True:
        ...
        for i in range(len(r)):
            ...
            if tmp.change_array[i]:
                ...
                width,height  = int((1/3)*len(next[0])),int((1/3)*len(next)) #We notice some sort of noise with cascade when pixel image too big (haarcascade training set not large dim)
                ...
                if len(catsExt) == 0:
                    ...
                    if len(catsExt) != 0:
                        ...
                        if model.exist: name, neg = model.prediction(pred2)
                        else: name, neg = None, math.inf

                        if neg > 55:
                            cv2.imwrite('testing/'+str(neg)+'.jpg',pred2)
                            number += 1

                        if (tmpPet.status == 0 and neg >= 60) or (not model.exist): 
                            crop_array[nbOfPet].append(pred2)
                        tmpPet.name, tmpPet.conf, tmpPet.d = name, neg, 0.0000001
                        print('conf2 :'+str(neg)+', name:'+str(name))


                    if len(crop_array[nbOfPet]) >= 5:
                        tmpPet.status = 1
                        full.append('pet'+str(len(full)-1))

                        for i in range(5):
                            cv2.imwrite('tmp/'+str(number)+'.jpg',crop_array[nbOfPet][i])
                            number += 1
...
def main(): 
    ...
    if array[0] == '-r':
        ...
        if len(array) != 1 and len(array) != 2: print('Not the right nb of argument, this requires no extra arguments')
